...
const Footer = () => (
    <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
        ...
        <div className="flex justify-between items-center md:flex-row flex-col w-full pt-7 border-t border-t-[#3F3E45]">
            <p className="font-poppins text-[18px] leading-7 text-center text-dimWhite"> Copyright Ⓒ 2022 HooBank. All Rights Reserved.</p>
            <div className="flex flex-row md:mt-0 mt-4">
                {socialMedia.map((item, index) => (
                        <a key={item.id} href={item.link} className={`flex
                        items-center ${index !== socialMedia.length - 1 ? 'mr-4' : 'mr-0'}`}>
